...
function getSumBetweenNumbers(n1, n2) {
  return (n1 + n2) * Math.ceil((n2 - n1) / 2);
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  if (isStartIncluded) openBracket = '[';
  if (!isStartIncluded) openBracket = '(';
  if (isEndIncluded) closeBracket = ']';
  if (!isEndIncluded) closeBracket = ')';
...
function getDigitalRoot(num) {
  function splitIntoNumArray(n) {
    return n
      .toString()
      .split('')
      .map((e) => parseInt(e, 10));
  }
  const arr = splitIntoNumArray(num);
  let sum = arr.reduce((prev, curr) => prev + curr, 0);
  while (sum.toString().length > 1) {
    const tempArr = splitIntoNumArray(sum);
    sum = tempArr.reduce((prev, curr) => prev + curr, 0);
  }
  return sum;
}
...
function getCommonDirectoryPath(paths) {
  ...
  return first.substring(0, i).replace(/(?<=\/)\w+$/, '');
}
