...
function timeSpanToString(startDate, endDate) {
  ...
  const hours = String(Math.floor(delta / 3600000)).padStart(2, '0');
  const minutes = String(Math.floor((delta % 3600000) / 60000)).padStart(
    2,
    '0',
  );
  const seconds = String(
    Math.floor((delta - hours * 3600000 - minutes * 60000) / 1000),
  ).padStart(2, '0');
  const milliseconds = String(
    delta - hours * 3600000 - minutes * 60000 - seconds * 1000,
  ).padStart(3, '0');
...
function angleBetweenClockHands(date) {
  ...
  return (smallerAngle / 180) * Math.PI;
}
